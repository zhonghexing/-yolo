...
QTableWidget {{
    background: {COLOR_BG_ALT};
    alternate-background: {COLOR_SURFACE};
    border: 1px solid {COLOR_BORDER};
...
class SteelDefectApp(QMainWindow):
    ...
    def _init_ui(self):
        ...
        title = QLabel(f"{APP_TITLE_CN}  -  基于 YOLOv8 的智能表面缺陷检测")
        title.setStyleSheet(f"color: {COLOR_TEXT}; font-size: 13px; font-weight: 600;")
        header.addWidget(title)

        header.addStretch()

        version = QLabel(f"v{APP_VERSION}")
        version.setStyleSheet(f"color: {COLOR_TEXT_DIM}; font-size: 11px;")
    ...
    def _create_preview_panel(self):
        ...
        self.image_display.setMinimumSize(300, 200)
        ...
        self.image_info_label.setStyleSheet(f"""
            ...
            font-size: 10px;
        """)
        self.image_info_label.setFixedHeight(16)
    ...
    def _create_results_panel(self):
        ...
        pass_widget = self._create_stat_inline("合格", self._pass_count_label, COLOR_OK)
        stats_layout.addWidget(pass_widget)

        # 不合格
        self._fail_count_label = QLabel("0")
        fail_widget = self._create_stat_inline("不合格", self._fail_count_label, COLOR_ERR)
        stats_layout.addWidget(fail_widget)

        # 总数
        self._total_count_label = QLabel("0")
        total_widget = self._create_stat_inline("总数", self._total_count_label, COLOR_ACCENT)
        stats_layout.addWidget(total_widget)

        # 合格率
        self._rate_label = QLabel("0%")
        rate_widget = self._create_stat_inline("合格率", self._rate_label, COLOR_WARN)
        stats_layout.addWidget(rate_widget)

        stats_layout.addStretch()

        # 检测数量标签
        self.detection_count_badge = QLabel("0 项")
        self.detection_count_badge.setStyleSheet(f"""
            ...
            font-weight: 600;
        """)
        stats_layout.addWidget(self.detection_count_badge)

        layout.addWidget(stats_bar)

        # ── 下方：表格 + 缺陷分布 ──
        bottom_split = QHBoxLayout()
        bottom_split.setSpacing(4)
        ...
        bottom_split.addWidget(table_container, 1)

        # 缺陷分布（右侧窄条）
        defect_container = QFrame()
        defect_container.setObjectName("card")
        defect_container.setMaximumWidth(180)
        defect_layout = QVBoxLayout(defect_container)
        defect_layout.setContentsMargins(8, 8, 8, 8)
        defect_layout.setSpacing(3)
        ...
        bottom_split.addWidget(defect_container)
    ...
    def _open_directory(self):
        ...
        if dir_path:
            ...
            if image_files:
                ...
                self.image_info_label.setText(f"文件夹: {Path(dir_path).name} ({len(image_files)} 张图片)")
    ...
    def _update_result_table(self, result):
        ...
        self.detection_count_badge.setText(f"{len(result.detections)} 项")
    ...
    def _clear_results(self):
        ...
        self.detection_count_badge.setText("0")
